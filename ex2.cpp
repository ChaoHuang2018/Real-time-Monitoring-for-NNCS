...
vector<Result_Info> run_ex2(string nn_name, string act_name, string trajectory_file_name)
{
    clock_t begin, end;
    begin = clock();
    ...
    Interval x0(-2, 2), x1(-2, 2), x2(-2, 2), control(-1, 1), disturbance(-0.1, 0.1);
    ...
    for (int i = 1; i < traces.size(); i++)
    {
        ...
        for (int j = 1; j < traces[i].size(); j++)
        {
            ...
            remainderEval_benchmark2(remainder, stateSpace, u, d, 1);
            int N = 2;
            for (int i = 0; i < N; ++i)
            {
                stateSpace = domain;
                remainderEval_benchmark2(remainder, stateSpace, u, d, 1);
                linearizationDomainEval_benchmark2(domain, x_current, remainder, u, d, 1);
            }
            ...
            dynamics_linear_taylor_benchmark2(dynamics_string, dynamics_coeff, dynamics_const_term, domain);
            remainder_linear_taylor_benchmark2(linearization_remainder, domain);
            ...
            for (int s = 0; s < numVars; s++)
            {
                nn_coeff[2][s] = nn_taylor.get_jacobian()[s];
            }
...
int test_ex2()
{
    string nn_name = "nn/nn_2_sigmoid";
    string act_name = "sigmoid";
    string trajectory_file_name = "trajectories/nn_2_sigmoid";

    vector<Result_Info> result_list = run_ex2(nn_name, act_name, trajectory_file_name);
