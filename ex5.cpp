...
vector<Result_Info> run_ex5(string nn_name, string act_name, string trajectory_file_name)
{
    clock_t begin, end;
    begin = clock();
    ...
    for (int i = 1; i < traces.size(); i++)
    {
        ...
        for (int j = 1; j < traces[i].size(); j++)
        {
            ...
            for (int i = 0; i < N; ++i)
            {
                stateSpace = domain;
                remainderEval_benchmark5(remainder, stateSpace, u, d, 1);
                linearizationDomainEval_benchmark5(domain, x_current, remainder, u, d, 1);
            }
...
int test_ex5()
{
    string nn_name = "nn/nn_5_sigmoid";
    string act_name = "sigmoid";
    string trajectory_file_name = "trajectories/nn_5_sigmoid";

    vector<Result_Info> result_list = run_ex5(nn_name, act_name, trajectory_file_name);
